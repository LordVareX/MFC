...
public:
    FIdentityProviderType &fromAWS(const Aws::CognitoIdentityProvider::Model::IdentityProviderType &awsIdentityProviderType) {
        ...
        for (const std::pair<Aws::String, Aws::String>& elem : awsIdentityProviderType.GetProviderDetails()) {
            this->providerDetails.Add(elem.first.c_str(), elem.second.c_str());
        }

        this->attributeMapping.Empty();
        for (const std::pair<Aws::String, Aws::String>& elem : awsIdentityProviderType.GetAttributeMapping()) {
        ...
        this->lastModifiedDate = FDateTime::FromUnixTimestamp(awsIdentityProviderType.GetLastModifiedDate().Millis());

        this->creationDate = FDateTime::FromUnixTimestamp(awsIdentityProviderType.GetCreationDate().Millis());
