...
public:
    FUserImportJobType &fromAWS(const Aws::CognitoIdentityProvider::Model::UserImportJobType &awsUserImportJobType) {
        ...
        this->creationDate = FDateTime::FromUnixTimestamp(awsUserImportJobType.GetCreationDate().Millis());

        this->startDate = FDateTime::FromUnixTimestamp(awsUserImportJobType.GetStartDate().Millis());

        this->completionDate = FDateTime::FromUnixTimestamp(awsUserImportJobType.GetCompletionDate().Millis());
        ...
        this->importedUsers = (int)awsUserImportJobType.GetImportedUsers();

        this->skippedUsers = (int)awsUserImportJobType.GetSkippedUsers();

        this->failedUsers = (int)awsUserImportJobType.GetFailedUsers();
