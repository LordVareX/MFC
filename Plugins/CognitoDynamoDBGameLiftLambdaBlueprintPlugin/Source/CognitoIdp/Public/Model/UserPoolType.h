...
public:
    FUserPoolType &fromAWS(const Aws::CognitoIdentityProvider::Model::UserPoolType &awsUserPoolType) {
        ...
        this->lastModifiedDate = FDateTime::FromUnixTimestamp(awsUserPoolType.GetLastModifiedDate().Millis());

        this->creationDate = FDateTime::FromUnixTimestamp(awsUserPoolType.GetCreationDate().Millis());
        ...
        for (const std::pair<Aws::String, Aws::String>& elem : awsUserPoolType.GetUserPoolTags()) {
