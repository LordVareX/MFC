...
public:
    FGetIdentityPoolRolesResult &fromAWS(const Aws::CognitoIdentity::Model::GetIdentityPoolRolesResult &awsGetIdentityPoolRolesResult) {
        ...
        this->roles.Empty();
        for (const std::pair<Aws::String, Aws::String>& elem : awsGetIdentityPoolRolesResult.GetRoles()) {
            this->roles.Add(elem.first.c_str(), elem.second.c_str());
        }

        this->roleMappings.Empty();
        for (const std::pair<Aws::String, Aws::CognitoIdentity::Model::RoleMapping>& elem : awsGetIdentityPoolRolesResult.GetRoleMappings()) {
