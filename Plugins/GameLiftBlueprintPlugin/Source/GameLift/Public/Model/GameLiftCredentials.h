...
public:
    FGameLiftCredentials &fromAWS(const Aws::GameLift::Model::AwsCredentials &awsGameLiftCredentials) {
        ...
        this->accessKeyId = awsGameLiftCredentials.GetAccessKeyId().c_str();

        this->secretAccessKey = awsGameLiftCredentials.GetSecretAccessKey().c_str();

        this->sessionToken = awsGameLiftCredentials.GetSessionToken().c_str();
