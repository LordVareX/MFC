...
public:
    FVpcPeeringConnection &fromAWS(const Aws::GameLift::Model::VpcPeeringConnection &awsVpcPeeringConnection) {
        this->fleetId = awsVpcPeeringConnection.GetFleetId().c_str();

        this->ipV4CidrBlock = awsVpcPeeringConnection.GetIpV4CidrBlock().c_str();

        this->vpcPeeringConnectionId = awsVpcPeeringConnection.GetVpcPeeringConnectionId().c_str();

        this->status.fromAWS(awsVpcPeeringConnection.GetStatus());

        this->peerVpcId = awsVpcPeeringConnection.GetPeerVpcId().c_str();

        this->gameLiftVpcId = awsVpcPeeringConnection.GetGameLiftVpcId().c_str();
