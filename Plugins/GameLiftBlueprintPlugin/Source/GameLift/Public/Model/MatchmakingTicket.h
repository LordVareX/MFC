...
public:
    FMatchmakingTicket &fromAWS(const Aws::GameLift::Model::MatchmakingTicket &awsMatchmakingTicket) {
        this->ticketId = awsMatchmakingTicket.GetTicketId().c_str();

        this->configurationName = awsMatchmakingTicket.GetConfigurationName().c_str();
        ...
        this->statusReason = awsMatchmakingTicket.GetStatusReason().c_str();

        this->statusMessage = awsMatchmakingTicket.GetStatusMessage().c_str();

        this->startTime = FDateTime::FromUnixTimestamp(awsMatchmakingTicket.GetStartTime().Millis());

        this->endTime = FDateTime::FromUnixTimestamp(awsMatchmakingTicket.GetEndTime().Millis());
