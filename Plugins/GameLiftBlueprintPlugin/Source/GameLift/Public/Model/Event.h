...
public:
    FAWSEvent &fromAWS(const Aws::GameLift::Model::Event &awsEvent) {
        this->eventId = awsEvent.GetEventId().c_str();

        this->resourceId = awsEvent.GetResourceId().c_str();
        ...
        this->message = awsEvent.GetMessage().c_str();

        this->eventTime = FDateTime::FromUnixTimestamp(awsEvent.GetEventTime().Millis());

        this->preSignedLogUrl = awsEvent.GetPreSignedLogUrl().c_str();
