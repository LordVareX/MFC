...
public:
    ...
    FAWSPlayer &fromAWS(const Aws::GameLift::Model::Player &awsPlayer) {
        this->playerId = awsPlayer.GetPlayerId().c_str();

        this->playerAttributes.Empty();
        for (const std::pair<Aws::String, Aws::GameLift::Model::AttributeValue> elem : awsPlayer.GetPlayerAttributes()) {
            this->playerAttributes.Add(elem.first.c_str(), FGameLiftAttributeValue().fromAWS(elem.second));
        }

        this->team = awsPlayer.GetTeam().c_str();

        this->latencyInMs.Empty();
        for (const std::pair<Aws::String, int> elem : awsPlayer.GetLatencyInMs()) {
