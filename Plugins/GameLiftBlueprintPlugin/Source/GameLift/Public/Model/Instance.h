...
public:
    FInstance &fromAWS(const Aws::GameLift::Model::Instance &awsInstance) {
        this->fleetId = awsInstance.GetFleetId().c_str();

        this->instanceId = awsInstance.GetInstanceId().c_str();

        this->ipAddress = awsInstance.GetIpAddress().c_str();

        this->dnsName = awsInstance.GetDnsName().c_str();
