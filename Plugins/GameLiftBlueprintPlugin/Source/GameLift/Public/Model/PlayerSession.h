...
public:
    FAWSPlayerSession &fromAWS(const Aws::GameLift::Model::PlayerSession &awsPlayerSession) {
        this->playerSessionId = awsPlayerSession.GetPlayerSessionId().c_str();

        this->playerId = awsPlayerSession.GetPlayerId().c_str();

        this->gameSessionId = awsPlayerSession.GetGameSessionId().c_str();

        this->fleetId = awsPlayerSession.GetFleetId().c_str();
        ...
        this->ipAddress = awsPlayerSession.GetIpAddress().c_str();

        this->dnsName = awsPlayerSession.GetDnsName().c_str();

        this->port = awsPlayerSession.GetPort();

        this->playerData = awsPlayerSession.GetPlayerData().c_str();
