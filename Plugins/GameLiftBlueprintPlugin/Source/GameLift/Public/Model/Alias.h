...
public:
    FAlias &fromAWS(const Aws::GameLift::Model::Alias &awsAlias) {
        ...
        this->aliasId = awsAlias.GetAliasId().c_str();

        this->name = awsAlias.GetName().c_str();

        this->aliasArn = awsAlias.GetAliasArn().c_str();

        this->description = awsAlias.GetDescription().c_str();

        this->routingStrategy.fromAWS(awsAlias.GetRoutingStrategy());

        this->creationTime = FDateTime::FromUnixTimestamp(awsAlias.GetCreationTime().Millis());

        this->lastUpdatedTime = FDateTime::FromUnixTimestamp(awsAlias.GetLastUpdatedTime().Millis());
