...
public:
    ...
    FS3Location &fromAWS(const Aws::GameLift::Model::S3Location &awsS3Location) {
        this->bucket = awsS3Location.GetBucket().c_str();

        this->key = awsS3Location.GetKey().c_str();

        this->roleArn = awsS3Location.GetRoleArn().c_str();

        this->objectVersion = awsS3Location.GetObjectVersion().c_str();
