...
public:
    FInstanceAccess &fromAWS(const Aws::GameLift::Model::InstanceAccess &awsInstanceAccess) {
        this->fleetId = awsInstanceAccess.GetFleetId().c_str();

        this->instanceId = awsInstanceAccess.GetInstanceId().c_str();

        this->ipAddress = awsInstanceAccess.GetIpAddress().c_str();
